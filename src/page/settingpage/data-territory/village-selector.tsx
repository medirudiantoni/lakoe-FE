...
import { fetchDistrict, fetchVillages } from "@/features/auth/services/data-territory";
...
export function VillageSelector({ districtId, selectedVillage, setSelectedVillage }: VillagesSelectorProps) {
  const [village, setvillage] = useState<{ id: string; name: string, postal_code:string }[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchDistrictData = async () => {
      ...
      try {
        ...
        if (response?.data) {
          setvillage(
            response.data.map((village: { name: string; code: string; postal_code:string }) => ({
              id: village.code,
              name: village.name,
    ...
    <Box>
        ...
        <Field label="Kode Pos" required>
          ...
          >
            <span className="font-normal">
              {loadingVillage ? "Loading..." : selectedVillage?.postal_code || "Pilih Kode Pos"}
            </span>
            <ChevronDown />
          </Button>
        </Field>
      ...
      {districtId && (
        <MenuContent position="absolute" width="full" maxHeight="200px" overflowY="auto" bg="white">
          {village.length > 0 ? (
            village.map((village) => (
              <MenuItem value="item" key={village.id} onClick={() => setSelectedVillage(village)}>
                {village.postal_code}
              </MenuItem>
            ))
          ) : (
            <MenuItem value="item" disabled>{loadingVillage ? "Memuat..." : "Tidak ada data tersedia"}</MenuItem>
          )}
        </MenuContent>
      )}
    </MenuRoot>
