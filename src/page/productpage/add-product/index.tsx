...
const AddProductForm = () => {
  ...
  const { fetchCategories, selectedCategoryId, setSelectedCategoryId } =
    useCategoryStore();
  const [getVariants, setGetVariants] = useState<VariantType[]>([]);
  const [theVariants, setTheVariants] = useState<VariantType[]>([]);
  const [productDataUpdateCase, setProductDataUpdateCase] =
  ...
  const [isVariantTypeAppears, setIsVariantTypeAppears] =
  ...
  function retrieveProductData(productId: string) {
    ...
    fetchProductById(productId, token!)
      .then((res) => {
        ...
        const getValues = () => {
          const options = product.variants?.find((item, id) =>
            id === 0 ? item.variantOptions : false
          );
          const optionValues = options?.variantOptions?.find((item, id) =>
            id === 0 ? item.variantOptionValues : false
          );
          const values = optionValues?.variantOptionValues?.find((item, id) =>
            id === 0 ? item : false
          );
          return values;
        };
  ...
  const onSubmit = async (data: ProductFormInputs) => {
    ...
    const filteredImageUpdateCase = previewImages.filter((e) =>
      e.includes('blob') ? false : e
    );
    ...
    <Box>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        {getVariants.length === 0 && (
          <ManageProductSectionForm errors={errors} register={register} />
        )}
        ...
        <ButtonsSectionForm
          isLoading={isLoading}
          isUpdateCase={productId ? true : false}
          data={previewImages}
        />
