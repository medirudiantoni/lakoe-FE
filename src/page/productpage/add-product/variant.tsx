...
const VariantSectionForm: React.FC<Props> = ({
  onChangeVariantData,
  setIsVariantType,
  isVariantType,
  productDataUpdateCase,
}) => {
  const { productId } = useParams();
  const [variantTypeNames, setVariantTypeNames] = useState<string[]>([]);
  const [variantData, setVariantData] = useState<VariantType[]>([]);
  const [allVariantsValues, setAllVariantsValues] =
    useState<VariantOptionValueType | null>(null);
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (
        JSON.stringify(currentCombinations) !== JSON.stringify(combinations)
      ) {
        setVariantData((prev) => {
    ...
    setVariantData((prev) => {
      ...
      if (existingVariant) {
        if (prev.length === 2) {
          ...
          return prev.filter(
            (item) => item.name !== variant && item.name !== 'final'
          );
        }
      } else {
        const newVariant = {
  ...
  const handleDataVariant = useCallback(
    (data: VariantOptionType[], name: string, isActive: boolean) => {
      setVariantData((prev) => {
        const newState = [...prev];
  ...
  const handleDataOptionValues = useCallback(
    (optionName: string, data: VariantOptionValueType) => {
          ...
          variant.variantOptions?.map((option) => {
            option.variantOptionValues?.map((val) => {
              (val.isActive = data.isActive),
                (val.sku = `${data.sku.toUpperCase()}-${Array.from(option.name)
                  .map((e) => (e === ' ' ? '-' : e))
                  .join('')
                  .toUpperCase()}`),
  ...
  return (
    ...
    >
      ...
      >
        ...
        <HStack>
          <Button
            onClick={() => setIsVariantType(!isVariantType)}
            variant={'outline'}
            borderRadius={'50px'}
          >
            <CirclePlus />
          ...
          {variantData.length > 1 && (
            <DialogRoot role="alertdialog">
              ...
              <DialogContent>
                ...
                <DialogBody>
                  <p>
                    Jika dihapus maka data tidak dapat dipulihkan dan anda akan
                    membuat data ulang dari awal jika ingin membuat varian
                    produk
                  </p>
                </DialogBody>
                <DialogFooter>
                  ...
                  <Button colorPalette="red" onClick={handleResetAllData}>
                    Hapus
                  </Button>
                </DialogFooter>
                <DialogCloseTrigger />
        ...
        (variant, id) =>
          variant.name !== 'final' && (
            <ItemVariantTypeForm
              ...
              label={variant.name}
              onDataChange={(data) =>
                handleDataVariant(data, variant.name, true)
              }
        ...
        variant.name === 'final'
          ? variant.variantOptions?.map((option, id) => (
              <VariantOptionValuesForm
                ...
                dataPlaceHolder={
                  option.variantOptionValues && option.variantOptionValues[0]
                }
              />
            ))
          : null
