...
interface Props {
    label: string;
    index: number;
    onDataChange?: (data: VariantOptions[]) => void;
}
...
const VariantOptionsForm: React.FC<Props> = ({ label, onDataChange }) => {
    ...
    const [variantOptionNames, setVariantOptionNames] = useState<VariantOptions[]>([]);

    useEffect(() => {
        onDataChange &&
            onDataChange(variantOptionNames);
    }, [variantOptionNames]);
    ...
    const addInputValue = (event: React.KeyboardEvent<HTMLInputElement>, value: string) => {
        if (event.key === "Enter" && value.trim()) {
            ...
            setVariantOptionNames(prev => [...prev, {
                name: value,
                variantOptionValues: []
            }]);
            setInputText("");
            ...
            <Field label={label} mt={5}>
                <HStack w="full" px={2} borderWidth={1} borderColor="gray.200" borderRadius="md">
                    {variantOptionNames.map((item, id) => (
                        <HStack key={id} gap={1} pl="2" pr="0.5" bgColor="gray.200" borderRadius={4}>
                            <p>{item.name}</p>
