...
const ProductCard: React.FC<ProductCardProps> = ( props ) => {
    ...
    function showThePrice(){
        const finalIndex = Number(props.product.variants?.findIndex(e => e.name === "final"));
        const prices: number[] = [];
        props.product.variants && props.product.variants[finalIndex].variantOptions?.map(e => {
    ...
    return (
        <Box role="button"
            ...
            <VStack alignItems="start" justifyContent="space-between" className="font-poppins">
                <Box mb="1">
                    <Heading size="lg" fontWeight="semibold">{props.product.name}</Heading>
                    <Text fontSize="xs" color="gray.600">{props.product.category?.name}</Text>
                </Box>
