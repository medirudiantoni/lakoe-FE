...
const CategoryDropDown: React.FC<CategoryDropDownProps> = ({
  ...
  useEffect(() => {
    axios
      .get<CategoryType[]>(`${apiURL}category`)
      .then((res) => {
  ...
  useEffect(() => {
    setSubCategory(
      category?.find((cat) => cat.name === selectedCategory?.name)?.children ||
        null
    );
  ...
  useEffect(() => {
    setSubSubCategory(
      subCategory?.find((cat) => cat.name === selectedSubCategory?.name)
        ?.children || null
    );
  ...
  return (
    <Box
      className="group-hover:h-fit h-0 group-hover:py-5 overflow-hidden duration-100"
      position="absolute"
      top="100%"
      left={0}
      borderRadius="lg"
    ...
    >
      ...
      >
          ...
          {category &&
            category.map((cat: any, id: number) => (
              <Button
                ...
                bg={selectedCategory?.name === cat.name ? 'gray.200' : ''}
                color={selectedCategory?.name === cat.name ? 'blue.700' : ''}
                variant="outline"
                borderWidth={0}
        ...
        >
          {subCategory &&
            subCategory.map((cat: any, id: number) => (
              <Button
                key={id}
                onClick={() => handleSubCategorySelect(cat)}
                bg={selectedSubCategory?.name === cat.name ? 'gray.200' : ''}
                color={selectedSubCategory?.name === cat.name ? 'blue.700' : ''}
                variant="outline"
                borderWidth={0}
                justifyContent="space-between"
                w="full"
        ...
        >
          {subSubCategory &&
            subSubCategory.map((cat: any, id: number) => (
              <Button
                onClick={() => handleSubSubCategorySelect(cat)}
                bg={selectedSubSubCategory?.name === cat.name ? 'gray.200' : ''}
                color={
                  selectedSubSubCategory?.name === cat.name ? 'blue.700' : ''
                }
                key={id}
                variant="outline"
                borderWidth={0}
