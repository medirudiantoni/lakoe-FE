    ...
    <>
      {readyToShip.length > 0 ? (
        readyToShip.map((order, index) => {
          ...
          if (!product) {
            return (
              <Box key={index} width="full" border="1px solid" borderColor="gray.200" height="190px" borderRadius="10px" mt={3} p={3}>
                ...
                <Box>
                  ...
                  <Box display="flex" justifyContent="space-between" borderBottom="1px solid" borderColor="gray.300" pb={2}>
                    ...
                    <Button variant="outline" borderRadius="20px">
                      {order.status === 'Siap Dikirim' ? 'Kabari Pembeli' : 'Lihat Detail'}
                    </Button>
                  </Box>
