...
const PricingPage = () => {
  ...
  const handleRedirect = (name: string, price: string, limit: string) => {
    navigate(`/payment?name=${name}&price=${price}&limit=${limit}`);
  };
  return (
    <Box w="full" minH="100vh">
      ...
      <VStack>
        ...
        <Text fontSize="xl" maxW="lg" textAlign="center" mb="4">
          Mulai bangun bisnis dan tentukan paket yang tepat untuk kebutuhan anda
        </Text>
        <HStack flexWrap={{ base: 'wrap', lg: 'nowrap' }}>
          {pricings.map((item, id) => (
            ...
            >
              ...
              <Button
                onClick={() =>
                  handleRedirect(item.name, item.price, item.limit)
                }
                w="full"
                bg="blue.500"
                color="white"
