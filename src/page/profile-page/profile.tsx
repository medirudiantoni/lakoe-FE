...
export function Profile() {
  ...
  const mutation = useMutation({
    mutationFn: (data: {email?: string; phone?: string}) =>  updateUser(userId!, token!,data),
        onSuccess: ()=> {
      ...
      <Grid templateColumns="repeat(2, 1fr)">
        <GridItem mt={5}>
          ...
          <Field label="No telepon" mt={3}>
            <Input placeholder="Masukan no telepon" name='phone' value={formData.phone} disabled={!isEditing} onChange={handleChange} />
          </Field>
          <Button  mt={4} onClick={toggleEdit} colorPalette={isEditing ? 'red' : 'blue'}>
            {isEditing ? 'Batalkan' : 'Ubah'}
          </Button>
          {isEditing && (
            <Button mt={4} ml={2} colorPalette={'blue'} onClick={handleUpdate}>Perbarui</Button>
          )}
